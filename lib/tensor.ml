...
let get_tensor = function
  | Tensor t ->
      t
  | _ ->
      failwith "can only extract non singular tensor"
...
let init a input =
  ...
  let rec helper shape acc =
    ...
    | [] ->
        acc @@ scalar a
    | x :: xs ->
        helper xs (fun t -> List.init x (fun _ -> t) |> tensor)
  in
...
let print t =
  let rec helper = function
    ...
    | Tensor ts ->
        Printf.printf "[" ;
        Array.iter (fun t -> helper t ; Printf.printf ", ") ts ;
        Printf.printf "]"
  in
  helper t ; print_newline ()
...
let rec update t indices value =
  ...
  | [], Scalar _ ->
      value
  | x :: [], Tensor ts ->
      let copy_ts = Array.copy ts in
      copy_ts.(x) <- value ;
      Tensor copy_ts
  | x :: ys, Tensor ts ->
      let copy_ts = Array.copy ts in
      copy_ts.(x) <- update copy_ts.(x) ys value ;
      Tensor copy_ts
  | _, _ ->
...
let mapi (f : int list -> float -> t) (l : t) : t =
  let rec helper tensor r access_list =
    if r = 1 then
      let array_t = get_tensor tensor in
      Array.mapi (fun i x -> f (access_list @ [i]) (get_value x)) array_t
      |> mk_tensor
    else if r = 0 then f [] (get_value tensor)
    else
      let array_t = get_tensor tensor in
      Array.mapi (fun i x -> helper x (r - 1) (access_list @ [i])) array_t
      |> mk_tensor
  in
  helper l (rank l) []

let%test "test_mapi simple" =
  let t1 = floats [0.; 0.] in
  let t2 = floats [0.; 0.] in
  let t3 = tensor [t1; t2] in
  let f _ _ = scalar 2.0 in
  let t4 = mapi f t3 in
  print t4 ;
  let expect = tensor [floats [2.; 2.]; floats [2.; 2.]] in
  expect = t4

let%test "test_mapi complicated" =
  (* let t1 = floats [0.; 0.] in *)
  (* let t2 = floats [0.; 0.] in *)
  let t3 = floats [0.; 0.] in
  print (update t3 [1] (scalar 99.0));
  print t3;
  let f indices x = List.iter (fun x -> Printf.printf "%d " x) indices ; update t3 indices (scalar @@ x +. 0.01) in
  let t4 = mapi f t3 in
  print t4;
  true
...
let small_increments t =
  ...
  | Tensor ts ->
      Array.mapi
        (fun i a ->
          let copy_ts = Array.copy ts in
          copy_ts.(i) <- scalar @@ (get_value a +. 0.01) ;
          Tensor copy_ts )
        ts
      |> Array.to_list
  | Scalar x ->
